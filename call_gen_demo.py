...
def call_table(intvl_st_time, intvl_call_count, aht_range):
    '''
    '''
    col_name = ['intvl_start_time','call_start_time', 'call_aht', 'call_end_time', 'call_st_time_elapsed', 'intvl_time_elapsed', 'call_type']
    ...
    for i in range(len(intvl_st_time)):
        ...
        for j in range(intvl_call_count[i]):
            #print(j)
            currCall = dynamicCall(aht_range, intvl_st_time[i])
            currCallStart = currCall.call_start_time
            currCallAHT = currCall.aht_actual
            int_df = pd.DataFrame({col_name[0]: intvl_st_time[i],
                                   ...
                                   col_name[5]: intvl_st_time[i].hour*3600 + intvl_st_time[i].minute*60 + intvl_st_time[i].second,
                                   col_name[6]: currCall.call_type
                                   }, index=[0])
            #print(int_df)
...
def agent_table(agent_count, call_table_df, use_cost_calculation=0, weight_idle=1, weight_dist=1, weight_switch=1, call_switch_agent_time=7.00):
    ...
    for i in range(len(call_table_df)):
        ...
        int_df = pd.DataFrame({col_name[0]: i,
                               ...
                               col_name[12]: call_table_df['call_type'][i]
                              }, index=[0])
        #print(int_df)
